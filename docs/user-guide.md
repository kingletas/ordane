...
> [!WARNING]
> **An evidence bundle is read as an excerpt, and a ledger missing its opening records is not.** The playbook's `make evidence` writes one release's records into a folder, beside a `chain.txt` naming the log they came from, how many records it held and where it ended. Ordane reads that folder as **An excerpt**: it checks the bundle's own checksums, then that every record `chain.txt` lists is present, still hashes to its own contents, and is the record that file recorded. It says which log the bundle claims it came from, and that the claim is the bundle's own word rather than anything it could check. It does not read the records as a chain, because they are a selection: anything deployed in between was left behind, so the numbering skips and the record after a gap links to one the bundle does not hold.
>
> **Only a path you name with `--ledger` can be a bundle.** A ledger Ordane found for itself, from `ledgers:` or from an inventory, is the control plane's own record: it is read as a chain, a missing start is a broken chain, and `ordane ledger` exits 2 on it. Nothing in a file can say whether its own missing start was a selection somebody asked for or a deletion somebody made, and the files that would say so sit in the same folder as the records, so anything able to write there could otherwise decide how a live ledger is read. `ordane --page ledger` opens the desktop console on this view.
