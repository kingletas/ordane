...
`ordane ledger --ledger PATH` reads a file you name instead of either, which is how you look at one somebody sent you. Point it at the `audit.jsonl` inside an evidence bundle and Ordane reads the whole folder: it checks the bundle's own checksums, then that every record the `chain.txt` lists is present, still hashes to its own contents, and is the record that file recorded. It calls that **An excerpt**, and it names the log the bundle says it was cut from so you can check it against that. **Only a path you name this way can be a bundle.** A ledger Ordane found for itself, from `ledgers:` or from an inventory, is the control plane's own record, and no file sitting beside it gets to change how it is read. What a record has to carry is [the fields it reads](user-guide.md#the-fields-it-reads).
