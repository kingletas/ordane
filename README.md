...
Where your deploy playbook keeps its own record of what it did, an append-only file with one line per step and each line carrying a hash of the line before it, the Ledger view reads that too. It answers the questions somebody asks afterwards: who deployed this, who approved it, what commit went out, was there a backup, did the site go into maintenance, and has anything in the record been changed since. Ordane never writes to that file, so a deploy somebody ran from a terminal shows up there as well as one launched from here. The keys it reads are listed in [the user guide](docs/user-guide.md#the-fields-it-reads).
