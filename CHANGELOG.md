...
- **An evidence bundle reads as an excerpt.** The playbook's `make evidence` writes one release's records into a folder beside a `chain.txt` naming the log they came from, how long it was and where it ended, and a `SHA256SUMS` over the lot. Ordane reads that folder as an excerpt: it checks the bundle's own checksums, then that every record `chain.txt` lists is present, still hashes to its own contents, is the record that file recorded, and is in order. It does not read the records as a chain, because they are a selection: anything deployed in between was left behind, so the numbering skips and the record after a gap links to one the bundle does not hold. It says which log the bundle claims it came from, and says that the claim is the bundle's own word.
- **Only a ledger you name with `--ledger` can be an evidence bundle.** One Ordane found for itself, from `ledgers:` or from an inventory, is the control plane's own record: it is read as a chain, and a missing start is a broken chain. The files that mark a folder as a bundle sit in the same directory as the records, so anything able to write that directory could otherwise decide how a live ledger is read.
...
- **A Ledger place, `Ctrl+7`.** It reads the playbook's append-only, hash-chained log and never writes to it, so a deploy somebody ran from a terminal appears alongside one launched from here.
...
- A deploy opens on who stands behind it (who requested it, who approved it, which host built it, who verified it afterwards), then the flow step by step, then ten answers read from the records alone, among them: what went out, whether it is what was approved and built, whether the site went into maintenance, whether a backup was taken or failed, how it ended, and whether the records can be trusted. A name is printed on a step only where it says something, so the step somebody else took stands out instead of being lost in a column of the same address.
- **The trust answer says what a hash chain proves and no more.** It reads **Nothing in the file was changed**. A chain cannot show records cut from the end, a file rewritten whole by whoever holds the writer, or who anybody actually was, and the guide names all three.
- **The same answers, graded the same way, in all three front ends.** The window, the browser console and the terminal each show what the engine computed, including how serious it judged each answer to be, and each prints the exact checksums and release ids so a reader can copy one rather than retype it.
- `ordane ledger` prints the newest deploys per ledger, and `ordane ledger last` or a release id prints one in full. Its exit status is meant for a scheduled job: **0** when every chain it read is intact, **2** when one is broken, and **1** when it could read nothing at all or found a file that would not open. A path with no file at it does not fail the job, because an environment that has never deployed looks exactly the same from here; the listing names those paths either way.
...
- A deploy whose `deploy.finished` carries no outcome reads as **Finished**, and one whose record says otherwise as **Did not succeed**. Records with no `deploy.started` behind them are gathered into one group called **Records only**, because a warm-up suite leaves several and none of them is a deploy.
- A ledger written in a format newer than this reader knows is named as such, rather than read as though the fields it does not know were empty.
- The user guide says what a full record looks like: what writes each record, and what an absence means. Three of them are environment settings rather than faults, and Ordane cannot tell a setting from a failure.
