...
- `ordane ledger` prints the newest deploys per ledger and exits 2 when a chain is broken, 1 when it found nothing to read and 0 when every chain is intact, so it can run from a scheduled job. `ordane ledger last`, or a release id, prints one deploy in full. The browser console has the same two pages.
...
- **The trust answer says what a hash chain proves and no more.** It read *"Yes. All N records link back to the start"*, which claims N is all of them and the names in them are real. A chain cannot show records cut from the end, a file rewritten whole by whoever holds the writer, or who anybody actually was. It now reads **Nothing in the file was changed**, and the guide names all three.
- A ledger written in a format newer than this reader knows is named as such rather than read silently. Fields it does not know were coming back empty under a green chain.
- `ordane ledger` exits **1** when it read nothing at all, so a mistyped or moved path fails a scheduled job instead of passing it for ever. A ledger that exists and holds no deploys yet is a fresh environment, and stays 0.
- The browser renders the severity the engine computed. An answer graded a problem, and a broken chain, were drawn in the class used to play something down, so a tampered ledger looked quieter than a clean one. It also carries the exact values and a break banner, which only the other two had.
- A test walks every `Answer`, `Step` and `Timing` the engine produces and looks for each in all three front ends, including the grading. That is how `level` and `exact` went missing from one of them.
